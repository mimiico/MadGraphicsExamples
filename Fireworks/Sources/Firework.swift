...
struct Firework {
    ...
    mutating func update(_ layer: Layer) -> Bool {
        if exploded {
            ...
            for spark in sparks {
                let color = Color.blend(foreground: color.rawValue, background: Color.black.rawValue, with: spark.lifespan)
                layer.draw() { canvas in
                    canvas.fillCircle(at: spark.pos, radius: size, data: color)
                }
            }
        } else {
            layer.draw() { canvas in
